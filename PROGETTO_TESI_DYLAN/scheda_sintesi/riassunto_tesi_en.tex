...
\begin{center}
  ...
  {\small Candidate: Dylan Magliano (s337915) \quad---\quad
   Supervisors: prof. Flavio Giobergia, Dr. Sergio Caprioli,
   Dr. Emanuele Cagliero}\\
...
Kullback--Leibler regularisation term: the objective used is a VAE-type
objective, composed of the Jeffreys divergence in the matrix domain and the
Kullback--Leibler divergence in the latent space, with unit weight
($\beta = 1$).
...
conformity to the prior must be satisfied. The choice of $K = 20$ --- made
before the two supplementary runs, at the elbow of the deterministic models'
reconstruction curve --- is therefore not an empirical optimum for the VAE (at
$K = 10$ the reconstruction error is in fact slightly lower) but a
methodological compromise which these runs show to be of little consequence.
...
more specific: to build a distribution of synthetic scenarios of the
correlation structure \emph{at a nominal 21-day horizon starting from the
market state observed today}, with which to estimate the possible variability
of correlations from a risk perspective. The technique
...
correlation matrices. The most important result is that the Cholesky
