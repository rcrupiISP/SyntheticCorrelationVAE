...
**Validazione dei 5 stylized facts sugli scenari generati** (notebook 14): per un singolo scenario alla volta (indici esplorati: 0, 1, 10, 63, 99, 312, 500, 705 a seconda della run), si confronta la matrice "oggi reale" con la matrice "futuro sintetico" su tutte e 5 le proprietà. **Nessuna violazione** è stata osservata negli scenari esaminati (PSD sempre verificata, Perron-Frobenius sempre soddisfatta, spettro MP con lo stesso numero di modalità di mercato/settore, dendrogrammi e MST topologicamente simili).

Due scenari sono stati ispezionati direttamente (immagini PNG, non solo stdout) come esempi di dettaglio, su due run diverse:
- **idx=705, run `_10_loss` (1000 scenari)**: γ_oggi=−2.119 vs γ_futuro=−2.236 (MST), 15 vs 14 connessioni massime, 55.0% vs 55.2% nodi foglia. Scelto (nel notebook) come **lo scenario più estremo in assoluto** secondo la distanza di Mahalanobis dal centroide (distanza 14.14 contro una media di 3.95 e un 95° percentile di 7.42) — cioè il meno rappresentativo dell'insieme. Questa run non è quella scelta come VAE definitivo (vedi B.7 risolto: il modello finale è `VAE_20dim_cholesky_06_loss`).
- **idx=63, run `_06_loss` (1000 scenari, il modello VAE definitivo)**: verificato tramite ispezione diretta delle immagini (non solo stdout testuale). Distribuzione pairwise: media/mediana 0.4512/0.4593 (oggi) vs 0.4635/0.4711 (futuro). Spettro MP: 9 autovalori sopra il limite in entrambe le matrici, $\lambda_1$=169.1 (oggi) vs 173.2 (futuro). **Perron-Frobenius: similarità coseno tra gli autovettori oggi/futuro pari a 0.9999 (market mode), 0.9893 e 0.9883 (sector mode 1 e 2)** — tutte le componenti del market mode restano dello stesso segno in entrambe le matrici. MST: γ_oggi=−2.119 vs γ_futuro=−2.227. Questo scenario è stato scelto perché si colloca al **95° percentile lungo il primo asse principale (PC1, 23.4% varianza)** della nuvola di 1000 scenari — un'analisi statistica distinta da quella di Mahalanobis, presente nello stesso notebook (l'anomalia massima per Mahalanobis in questa run è un altro scenario, idx=312).

**Limite di rigore importante**: questa validazione è **sempre su un singolo scenario per volta**, mai in forma aggregata/statistica sull'intero insieme di 100 o 1000 scenari generati. Non esiste nel codice un loop che calcoli, ad esempio, "su quanti dei 1000 scenari il primo autovettore ha tutte componenti positive" o la distribuzione aggregata dell'esponente γ del power-law. Un'analisi di questo tipo sarebbe l'evidenza naturale per rafforzare il Cap. 8, ma **non è stata implementata** (limite dichiarato esplicitamente nel testo del capitolo).

Analisi aggiuntive presenti: distanza di Mahalanobis della nuvola di scenari dal centroide, PCA sulla nuvola di scenari futuri, identificazione degli scenari al 5°/95° percentile lungo l'asse principale.
...
- **AGGIORNAMENTO (2026-07-06): scritti `Parte_III/capitolo8.tex`** (Generazione Stocastica, basato su A.9, scenario idx=63/`_06_loss`) **e `Parte_III/capitolo9_conclusioni.tex`** (Conclusioni e Sviluppi Futuri). La tesi compila correttamente (pdflatex+bibtex, 68 pagine, zero riferimenti non definiti).
