...
VAE (Jeffreys, Cholesky) & --- & --- & $1.55\times10^{-4}$ & --- \\
\hline
\end{tabular}
...
Il passaggio al Variational Autoencoder (VAE) introduce una dinamica prestazionale più sfumata di quanto una prima intuizione potrebbe suggerire, e dipende in modo cruciale dal dominio di rappresentazione della matrice (Sezione~\ref{sec:setup_vae}). Nel dominio raw (correlazione grezza), il VAE registra un incremento marcato dell'errore di ricostruzione rispetto a tutti gli altri modelli, PCA e Linear AE inclusi (a $K=20$ sul \emph{Test Set}, MSE pari a $6.33\times10^{-3}$ contro $1.78\times10^{-4}$ di PCA e Linear AE). Tuttavia, nel dominio Cholesky --- quello adottato in via definitiva per la parametrizzazione PSD-aware (Capitolo~\ref{ch:modelli_generativi}) --- il quadro si inverte parzialmente: il VAE (variante con loss basata sulla Divergenza di Jeffreys, Sezione~\ref{sec:setup_vae}) ottiene a $K=20$ un MSE di test pari a $1.55\times10^{-4}$, \textbf{inferiore} sia a quello della PCA ($2.73\times10^{-4}$) sia a quello del Linear AE ($3.38\times10^{-4}$), pur restando superiore a quello dell'Autoencoder Profondo deterministico ($7.19\times10^{-5}$).
